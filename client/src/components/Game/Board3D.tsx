...
export const Board3D = ({ onPieceSelect }: Board3DProps) => {
  ...
  const isPromotionMove = (from: string, to: string): number | null => {
    ...
    if (
      (piece.color === 0 && toRank === 8) ||
      (piece.color === 1 && toRank === 1)
    ) {
      return piece.color;
    }
    return null;
  ...
  const handleSquareClick = async (alg: string) => {
    ...
    if (selectedPos) {
      ...
      // Clicking own piece switches selection if not using an ability
      if (
        clickedPiece &&
        selectedPiece &&
  ...
  const handlePieceClick = (pos: string, e: any) => {
    ...
    // Can only select pieces of current turn (and not active an ability)
    if (
      clickedPiece &&
    ...
    <>
      <group>
        ...
        {game?.pieces.map((p, idx) => {
          ...
          // Check if this piece was the target of a bounce attack this turn
          const isBounceTarget =
            game?.lastMoveIsBounce && game?.lastMoveTo === p.position;

          return (
            <group key={idx} onClick={(e) => handlePieceClick(p.position, e)}>
              <Piece3D type={p.type} color={p.color} position={p.position} />
              ...
              {(p.currentHP < p.maxHP || game?.isAttritionMode) &&
                !isSelected && (
                  <Html
                    position={[px, 0.9, pz]}
                    center
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: "100%",
                          background:
                            p.currentHP < p.maxHP * 0.3 ? "#ff3333" : "#33ff33",
                  ...
                  >
                    <div
                      style={{
                        ...
                        fontSize: "24px",
                        textShadow:
                          "0 2px 4px rgba(0,0,0,0.8), 0 0 10px rgba(255,0,0,0.5)",
      ...
      {/* Action Bar (Only shows when a piece is selected and it's our turn) */}
